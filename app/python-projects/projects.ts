...
export const projects: TrackerProject[] = [
  ...
  },
  {
    ...
    iteration:
      "The course’s original code used LangChain’s older OpenAI-tools agent pattern with two tools, add and show. **LangChain 1.4.2 no longer includes that pattern, so the code stopped running and I rebuilt the agent on v1’s create_agent**, working through the change with Claude as a coaching partner rather than pasting in a rewrite. Getting it running took more than one swapped import: a missing Todoist package, a broken API-key setting, and a model setting LangChain guessed wrong (it assumed Google’s Vertex AI until I passed the Gemini model object directly). Then I added a third tool, remove_task, which finds a task by its exact name and checks it off, and updated the system prompt so the agent knows removal exists. Keeping an agent working as the framework underneath it changes turned out to be as much of the job as building it.",
  },
  {
    number: 13,
    slug: "app-13",
